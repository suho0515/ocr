...
class OCR:
  ...
  def callback(self,data):
    ...
    try:
      ...
      # set roi
      #x=270; y=110; w=50; h=180
      #roi_img = dst[y:y+h, x:x+w]
      
      
      
      roi_img = self.get_roi(cv_image)
      ...
      if self.start_cnt < 5:
        ...
        if (self.result_max[0] != -1) and (self.result_max[1] != -1) and (self.result_max[2] != -1):
          result_full = self.result_max[0]*100 + self.result_max[1]*10 + self.result_max[2]
          rospy.loginfo('full:%s / 1:%s/ 2:%s/ 3:%s' % (result_full, str(self.result_max[0]), str(self.result_max[1]), str(self.result_max[2])))
